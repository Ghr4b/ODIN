...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_milk) {described_class.new('milk',16)}

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_milk.type).to eq('milk')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_milk.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_beverage) {described_class.new('cola')}
      # remove the 'x' before running this test
      it 'is full' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_bev) {described_class.new('cola',7)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_bev).not_to be_full
      end
    end
  end
end
