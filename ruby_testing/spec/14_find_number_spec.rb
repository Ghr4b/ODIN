...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_seven) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        allow(number_guessing).to receive(:value).and_return(8)
    ...
    context 'when min is 0 and max is 3' do
      subject(:game_one) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        allow(number_guessing).to receive(:value).and_return(1)
        guess = game_one.make_guess
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:game_over) { described_class.new(0, 9, random_number, 8) }
    ...
    context 'when guess and random_number are not equal' do
      let(:random_number) { double('random_number', value: 8) }
      let(:game_not_over) { described_class.new(0, 9, random_number, 3) }
